/* ============================================================
   AI Pronunciation Trainer — callbacks.js
   Improvements:
   - Dark / light mode toggle (persisted in localStorage)
   - Mic level visualizer (Web Audio API AnalyserNode)
   - Session history panel with rolling scores
   - Circular score ring animation
   - Loading overlay (instead of blocking UI silently)
   - Difficulty only applied on next "Next" press
   - ASR trainer cached per language on backend (no change needed here)
...
let currentSample = 0;
let currentScore = 0;
let sample_difficult = 1;     // default: easy
...
function resetScoreRing() {
  ...
  const ringText = document.getElementById('scoreRingText');
  if (ringFill) {
    ringFill.style.strokeDashoffset = '251.33';
    ringFill.style.stroke = 'var(--accent)';
  }
  if (ringText) {
    ringText.textContent = '—';
    ringText.style.fill = 'var(--text)';
  }
}
...
function drawViz() {
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    // Color: cyan when idle, red when recording
...
function UINotSupported() {
  setStatus('Browser unsupported', '');
  unblockUI();
}

function UIRecordingError() {
...
async function cacheSoundFiles() {
  ...
  try {
    ...
    console.warn('[Sounds] Could not cache sound files:', e);
  }
}
...
async function playSoundForAnswerAccuracy(accuracy) {
  ...
  if (accuracy < mediumScoreThreshold) {
    buf = accuracy < badScoreThreshold ? soundFileBad : soundFileOkay;
  }
  playback(buf);
}
...
const getNextSample = async () => {
  blockUI('Fetching next sentence...');
  resetScoreRing();
...
const changeLanguage = (language, generateNewSample = false) => {
  ...
  for (const v of voices) {
    if (v.lang.startsWith(language) && v.name === targetName) {
      voice_synth = v; languageFound = true; break;
    }
  }
...
const recordSample = async () => {
  // Snapshot reference text at the moment recording starts
  try {
    if (currentText) {
  ...
  // Block everything except mic
  ['playSampleAudio', 'playRecordedAudio', 'buttonNext'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.classList ? el.classList.add('disabled') : (el.disabled = true);
  });
...
const stopRecording = () => {
  ...
  showLoading('Processing audio… this may take a moment');
};
...
const startMediaDevice = async () => {
  try {
    ...
    // Init visualizer from the live stream
    ...
    mediaRecorder.onstop = async () => {
      ...
      // Get the reference text (use snapshot taken at recording start)
      ...
      try {
        ...
        // Add to session history BEFORE resetting reference
        ...
        // Color each word in the sentence
...
const playRecording = async (start = null, end = null) => {
  ...
  try {
    ...
    if (start == null || end == null) {
      ...
      audioRecorded.addEventListener('ended', () => {
        audioRecorded.currentTime = 0;
        unblockUI();
        setStatus('Recording played', '');
      }, { once: true });
    } else {
      ...
      setTimeout(() => {
        try { audioRecorded.pause(); audioRecorded.currentTime = 0; } catch (e) {}
        unblockUI();
        setStatus(page_title, '');
      }, Math.round((e - s) * 1000));
    }
  } catch (err) {
    console.error('[playRecording]', err);
    unblockUI();
  }
};
